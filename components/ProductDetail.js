...
export function renderProductDetail(product, { onClose } = {}) {
      ...
      <div class="product-detail-content">
        <div class="product-detail-image-col">
          <div class="carousel-container">
            <img class="product-detail-image" src="${carouselImages[0]}" alt="${product.name}" />
            <div class="carousel-dots">
              ${carouselImages.map((_, i) => `<span class="carousel-dot${i === 0 ? ' active' : ''}"></span>`).join('')}
            </div>
          </div>
        </div>
  ...
  function animateSlide(direction, nextIdx) {
    ...
    imageEl.classList.add(direction === 'left' ? 'slide-left-out' : 'slide-right-out');
    setTimeout(() => {
      imageEl.src = carouselImages[nextIdx];
      imageEl.classList.remove('slide-left-out', 'slide-right-out');
      imageEl.classList.add(direction === 'left' ? 'slide-left-in' : 'slide-right-in');
      setTimeout(() => {
